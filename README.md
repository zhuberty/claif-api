...
One can run `./src/claif-api/seed_db.sh` to manually start the FastAPI app, truncate/reset the tables, and seed the database.
...
## Database Schema:
The database schema for the CLAIF API is defined using SQLAlchemy ORM. The schema consists of the following models (generated by `src/claif-api/utils/schema.py`):
